...
export class CommonPreviewLtcClaimComponent implements OnInit {
  ...
  get personalFields() {
    return [
      ...
      { label: 'Journey Performed By', value: this.claim?.performedBy },
    ].filter((field) => !this.isBlank(field.value));
  }
  ...
  get claimFields() {
    return [
      ...
      { label: 'Place of Visit', value: this.claim?.placeOfVisit },
      { label: 'Place', value: this.claim?.currentPlace },
      { label: 'Journey Amount', value: this.amount(this.claim?.journeyAmt) },
  ...
  private parseClaimPreviewResponse(response: any): void {
    ...
    this.advance = this.firstItem(claims?.yatLtcAdvDTOs);
    this.travelRows = this.resolveTravelRows(claims);
    this.familyRows = this.asArray(this.claim?.yatFamilyDetailDTOs);
