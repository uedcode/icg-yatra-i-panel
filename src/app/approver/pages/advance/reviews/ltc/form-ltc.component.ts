...
export class FormLtcAdvanceComponent implements OnInit {
  ...
  inputDate(value: any): string {
    const formatted = this.asDate(value);
  ...
  get primaryAdvanceDetails(): any {
    return this.formObj?.yatLtcAdvDTOs?.[0];
  }
  ...
  get familyDetails(): any[] {
    return Array.isArray(this.formObj?.yatFamilyDetailDTOs)
      ? this.formObj.yatFamilyDetailDTOs
      : [];
  }

  get spouseDetails(): any {
    return this.advanceDetails?.yatSpouseDetailDTO || null;
  }

  get travelDetails(): any[] {
    return Array.isArray(this.formObj?.yatDtsDetailDTOs)
      ? this.formObj.yatDtsDetailDTOs
      : [];
  }

  get isNewlyMarried(): boolean {
    return String(this.advanceDetails?.isNewlyMarried || '').toUpperCase() === 'YES';
  }
