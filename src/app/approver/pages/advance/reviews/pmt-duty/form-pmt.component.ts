...
@Component({
  selector: 'app-approver-form-pmt',
  templateUrl: './form-pmt.component.html',
  styleUrls: ['./form-pmt.component.scss'],
  standalone: false,
})
export class FormPmtDutyComponent implements OnInit {
  ...
  get personalFields(): Array<{ label: string; value: any }> {
    return [
      { label: 'Name', value: this.advanceDetails?.name },
      { label: 'Rank', value: this.advanceDetails?.rank },
      { label: 'Personnel Number', value: this.advanceDetails?.pno },
      { label: 'Pay Level', value: this.advanceDetails?.payLevel },
      { label: 'Basic Pay (In Rs)', value: this.advanceDetails?.basicPay },
      { label: 'Permanent Transfer To', value: this.advanceDetails?.permTransTo },
      { label: 'From Station', value: this.advanceDetails?.stnFrom },
      { label: 'To Station', value: this.advanceDetails?.stnTo },
      { label: 'Distance between stations', value: this.advanceDetails?.distance },
      { label: 'Present Unit', value: this.advanceDetails?.videPresentUnit },
      { label: 'Applied To', value: this.advanceDetails?.appliedTo },
  ...
  openRelatedPreview(relatedClaimId: any): void {
    ...
    const url = `${this.$auth.getModuleName()}/preview-pmt-duty?id=${encodeURIComponent(relatedClaimId)}`;
    this.previewWindow.openUrl(url);
  }

  private getPreviewRoute(): string {
    return 'preview-pmt-duty';
  }
  ...
  get primaryAdvanceDetails(): any {
    return this.formObj?.yatPermDutyAdvDTOs?.[0];
  }
