...
export class ClaimFormLtcComponent implements OnInit {
  ...
  get allSectionsReviewed(): boolean {
    ...
    return this.form1 && this.form2 && this.form3 && this.form4 && this.form5 && bankReviewed && remarksReviewed;
  }
  ...
  get reviewTabs(): Array<{ key: string; label: string }> {
    const tabs = [
      ...
      { key: 'ship2', label: 'LTC Details' },
      { key: 'ship3', label: 'Travel Details' },
  ...
  get showLeaveDetails(): boolean {
    const familyType = String(this.claimDetails?.familyType ?? '').trim();
    return familyType !== '' && familyType !== 'Family';
  }

  get isNewlyMarried(): boolean {
    return String(this.claimDetails?.isNewlyMarried ?? '').trim() === 'Yes';
  }
  ...
  openPreview(): void {
    ...
    const url = `${this.$auth.getModuleName()}/preview-ltc-claim?id=${encodeURIComponent(this.claimId)}&subFormId=${encodeURIComponent(this.subFormId)}`;
  ...
  get claimDetails(): any {
    return this.formObj?.yatLtcClaimDTOs?.[0];
  }
  ...
  get spouseDetails(): any {
    return this.claimDetails?.yatSpouseDetailDTO;
  }

  get leaveDetails(): any {
    return this.claimDetails?.yatLtcClaimLeaveDTOs?.[0];
  }

  get leaveSubtypeRows(): any[] {
    return Array.isArray(this.claimDetails?.yatLtcClaimLeaveSubTypeDTOs) ? this.claimDetails.yatLtcClaimLeaveSubTypeDTOs : [];
  }
