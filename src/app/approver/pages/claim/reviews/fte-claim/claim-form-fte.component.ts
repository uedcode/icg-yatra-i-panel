...
export class ClaimFormFteComponent implements OnInit {
  ...
  get pageTitle(): string {
    return `${this.supplementaryId ? 'Supplementary ' : ''}FTE Claim`;
  }
  ...
  get allSectionsReviewed(): boolean {
    ...
    return this.form1 && this.form2 && this.form5 && this.form4 && bankReviewed && remarksReviewed;
  }
  ...
  get showDutyStation(): boolean {
    return !!String(this.claimDetails?.dutyStation ?? '').trim();
  }

  get showStationProceedingTo(): boolean {
    return !!String(this.claimDetails?.stationProceedingTo ?? '').trim();
  }

  get showPurposeOfDuty(): boolean {
    return !!String(this.claimDetails?.purpose ?? '').trim();
  }

  get showGuestDetails(): boolean {
    return !this.showPurposeOfDuty;
  }

  get purposeTypeText(): string {
    const purposeType = String(this.claimDetails?.purposeType ?? '').trim();
    return purposeType ? purposeType : 'Other';
  }
  ...
  openPreview(): void {
    ...
    const url = `${this.$auth.getModuleName()}/preview-fte-claim?id=${encodeURIComponent(this.claimId)}&subFormId=${encodeURIComponent(this.subFormId)}`;
  ...
  get claimDetails(): any {
    return this.formObj?.yatForeignDutyClaimDTOs?.[0];
  }
