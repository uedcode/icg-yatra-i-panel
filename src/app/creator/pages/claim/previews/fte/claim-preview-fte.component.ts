...
@Component({
  selector: 'app-claim-preview-fte',
  templateUrl: './claim-preview-fte.component.html',
  styleUrls: ['./claim-preview-fte.component.scss'],
  standalone: false,
})
export class ClaimPreviewFteComponent implements OnInit {
  readonly previewTitle = 'Requisition for Foreign Travel Claim';
  ...
  get personalFields() {
    return [
      ...
      { label: 'Probable Duration (Days)', value: this.claim?.duration || this.advance?.duration },
    ].filter((field) => !this.isBlank(field.value));
  }
  ...
  get claimFields() {
    return [
      ...
      { label: 'From Date', value: this.asDate(this.claim?.fromDate || this.claim?.tyFromDate) },
      { label: 'To Date', value: this.asDate(this.claim?.toDate || this.claim?.tyToDate) },
  ...
  private parseClaimPreviewResponse(response: any): void {
    ...
    this.formObj = claims;
    this.claim = this.firstItem(claims?.yatForeignDutyClaimDTOs || claims?.yatForeignDutyClaimDTO || claims?.foreignDutyClaimDTOs);
    this.advance = this.firstItem(claims?.yatForeignDutyAdvDTOs || claims?.yatForeignDutyAdvDTO);
