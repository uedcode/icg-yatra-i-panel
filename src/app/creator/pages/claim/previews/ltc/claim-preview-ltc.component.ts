...
@Component({
  selector: 'app-claim-preview-ltc',
...
export class ClaimPreviewLtcComponent implements OnInit {
  readonly previewTitle = 'Requisition for LTC Claim';
  ...
  get personalFields() {
    return [
      ...
      { label: 'LTC Type', value: this.claim?.ltcType || this.advance?.ltcType },
      { label: 'Block Year', value: this.claim?.blockYear || this.advance?.blockYear },
    ].filter((field) => !this.isBlank(field.value));
  }
  ...
  get claimFields() {
    return [
      ...
      { label: 'Place of Visit', value: this.claim?.placeOfVisit || this.claim?.ltcPlace || this.advance?.placeOfVisit },
      { label: 'From Date', value: this.asDate(this.claim?.fromDate || this.claim?.ltcFromDate) },
      { label: 'To Date', value: this.asDate(this.claim?.toDate || this.claim?.ltcToDate) },
  ...
  private parseClaimPreviewResponse(response: any): void {
    ...
    this.formObj = claims;
    this.claim = this.firstItem(claims?.yatLtcClaimDTOs || claims?.yatLtcClaimDTO || claims?.ltcClaimDTOs);
    this.advance = this.firstItem(claims?.yatLtcAdvDTOs || claims?.yatLtcAdvDTO);
