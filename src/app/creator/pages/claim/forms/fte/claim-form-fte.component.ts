...
/** YatForeignDutyClaimDTO */
interface YatForeignDutyClaimDTO {
...
interface ClaimsFteAdv {
  ...
  yatForeignDutyClaimDTOs: YatForeignDutyClaimDTO[];
...
export class ClaimFormFteComponent implements OnInit {
  activeTab: 'ship' | 'ship2' | 'ship4' | 'ship5' | 'ship6' | 'ship7' = 'ship';
  private readonly fteTabOrder: Array<'ship' | 'ship2' | 'ship4' | 'ship5' | 'ship6' | 'ship7'> = [
    'ship',
    'ship2',
    'ship4',
    'ship5',
  ...
  loadFtePurposeTypes(): void {
    const config = {
      headers: {
        subFormId: this.codeClaim?.fteClm || 'FTE',
      },
    };
  ...
  navigatePreview(route: string, id: any): void {
    const previewRoute = route || 'preview-fte-claim';
  ...
  private completeLegacyStatusTransition(status: string, claimId: string): void {
    if (!claimId) {
      this.$common.showMessage('Unable to update FTE Claim status.', 'danger');
      this.$common.hideLoader();
    ...
    this.$claimStateApi.changeStatusById(payload).subscribe(
      (res: any) => {
        if (res?.status === false) {
          this.$common.showMessage(
            res?.message || 'Unable to update FTE Claim status.',
            'danger'
          );
          this.$common.hideLoader();
          this.disableBtn = false;
      ...
      (err: any) => {
        console.error('Error while updating FTE Claim status', err);
        this.$common.showMessage(
          'Error while updating FTE Claim status.',
          'danger'
        );
        this.$common.hideLoader();
        this.disableBtn = false;
  ...
  //isIfscNull: any;
  activeSubFormId: string = this.codeClaim.fteClm;
  ...
  private initFromRoute(): void {
    ...
    this.activeSubFormId = this.codeClaim.fteClm;
  ...
  private createEmptyClaims(): ClaimsFteAdv {
    return {
      ...
      codeSubFormDTO: { subFormId: this.codeClaim.fteClm } as any,
      codeUnitDTO: { unit: '', descr: '' } as any,

      yatForeignDutyClaimDTOs: [this.createEmptyFteClaim()],
  ...
  private createEmptyFteClaim(): YatForeignDutyClaimDTO {
    return {
      ...
      this.$claimApi.getSingleClaim(config).subscribe(
        (response: any) => {
          ...
          if (!response || response.status !== true) {
            this.$common.showMessage(
              response?.message || 'Unable to load FTE Claim details.',
              'danger'
            );
            return;
          }
          ...
          const adv = this.claims.yatForeignDutyClaimDTOs[0];

          const dto =
            Array.isArray(obj.yatForeignDutyClaimDTOs) &&
            obj.yatForeignDutyClaimDTOs.length > 0
              ? obj.yatForeignDutyClaimDTOs[0]
              : null;
          ...
          this.calculateAmount(this.codeClaim.fteClm);
        },
        (error: any) => {
          this.$common.hideLoader();
          console.error('Error while loading FTE Claim details.', error);
          this.$common.showMessage(
            'Error while loading FTE Claim details.',
            'danger'
          );
        }
      );
  ...
  calculateAmount(type: string): void {
    if (type !== this.codeClaim.fteClm) return;
    this.recalcFteTotals();
  }
  ...
  validate(type: string, status: string): void {
    if (type !== this.codeClaim.fteClm && type !== this.activeSubFormId) return;
  ...
  setTab(tab: 'ship' | 'ship2' | 'ship4' | 'ship5' | 'ship6' | 'ship7'): void {
    this.activeTab = tab;
  }

  private activateTab(sectionId: string): void {
    if (this.fteTabOrder.includes(sectionId as any)) {
      this.setTab(sectionId as 'ship' | 'ship2' | 'ship4' | 'ship5' | 'ship6' | 'ship7');
    }
  }
  ...
  saveClaim(
    ...
    try {
      ...
      // enforce array
      if (!Array.isArray(tempClaim.yatForeignDutyClaimDTOs)) {
        tempClaim.yatForeignDutyClaimDTOs = tempClaim.yatForeignDutyClaimDTOs
          ? [tempClaim.yatForeignDutyClaimDTOs]
          : [];
      }
      ...
      tempClaim.yatDocsDTOs = this.claims.yatDocsDTOs || [];
      ...
      tempClaim.yatForeignDutyClaimDTOs.forEach((adv: any) => {
      ...
      this.$claimApi.createOrUpdateClaim(formData, null).subscribe(
        (res: any) => {
          if (!res || res?.status === false) {
            this.$common.showMessage(
              res?.message || 'Unable to save FTE Claim.',
              'danger'
            );
            this.$common.hideLoader();
            this.disableBtn = false;
        ...
        (err: any) => {
          console.error('Error while saving FTE Claim', err);
          this.$common.showMessage(
            'Error while saving FTE Claim.',
            'danger'
          );
          this.$common.hideLoader();
          this.disableBtn = false;
      ...
      console.error('Exception while saving FTE Claim', err);
      this.$common.showMessage(
        'Error while saving FTE Claim.',
        'danger'
      );
  ...
  submitFte(): void {
    ...
    this.saveClaim(this.codeClaim.fteClm, this.codeClaimState.outbox, true);
  }
  ...
  private runClientValidationOnly(): boolean {
    ...
    const sectionIds = ['ship', 'ship2', 'ship4', 'ship5', 'ship7', 'ship6'];
  ...
  private mergeSavedFteResponse(obj: any): void {
    ...
    const currentAdv = this.claims.yatForeignDutyClaimDTOs?.[0] || this.createEmptyFteClaim();
    const savedAdv = Array.isArray(obj.yatForeignDutyClaimDTOs)
      ? obj.yatForeignDutyClaimDTOs[0]
      : null;
    ...
    this.claims = {
      ...this.claims,
      ...obj,
      yatForeignDutyClaimDTOs: [{ ...currentAdv, ...(savedAdv || {}) }],
  ...
  private normalizeFtePayloadForSave(tempClaim: any): void {
    const adv = tempClaim?.yatForeignDutyClaimDTOs?.[0];
    if (adv) {
