...
interface Claims {
  ...
  yatPermDutyClaimDTOs: YatPermDutyClaimDTO[];
...
export class ClaimFormResettlementComponent implements OnInit {
  ...
  codeClaim = { pmtClm: 'PMT', resettleClm: 'RS' } as const;
  ...
  activeSubFormId: string = this.codeClaim.resettleClm;
  ...
  isCompositeChecked(): boolean {
    const adv = this.claims?.yatPermDutyClaimDTOs?.[0];
  ...
  private createEmptyClaims(): Claims {
    return {
      ...
      codeSubFormDTO: { subFormId: this.codeClaim.resettleClm } as any,
      codeUnitDTO: { unit: '', descr: '' } as any,

      yatPermDutyClaimDTOs: [this.createEmptyPmtClaim()],
      yatFamilyDetailDTOs: [],
      yatDtsDetailDTOs: [],
      yatDocsDTOs: [],
  ...
  private createEmptyPmtClaim(): YatPermDutyClaimDTO {
    return {
      permDutyAdvId: null,
      name: null,
      rank: null,
  ...
  private mergeSavedPmtResponse(obj: any): void {
    ...
    const currentAdv = this.claims?.yatPermDutyClaimDTOs?.[0] || this.createEmptyPmtClaim();
    const savedAdv = Array.isArray(obj.yatPermDutyClaimDTOs)
      ? obj.yatPermDutyClaimDTOs[0]
      : null;
    ...
    this.claims = {
      ...this.claims,
      ...obj,
      yatPermDutyClaimDTOs: [{ ...currentAdv, ...(savedAdv || {}) }],
  ...
  private completeSubmitStatusTransition(claimId: string): void {
    ...
    statusRequest.subscribe(
      ...
      (err: any) => {
        console.error('Error while updating Resettlement status', err);
        this.$common.showMessage('Error while updating Resettlement status.', 'danger');
        this.$common.hideLoader();
        this.disableBtn = false;
      }
    );
    ...
    try {
      ...
      this.$claimApi.getSingleClaim(this.config).subscribe(
        (response: any) => {
          ...
          const existingAdv = this.claims?.yatPermDutyClaimDTOs?.[0] || {};
          const dto =
            Array.isArray(obj.yatPermDutyClaimDTOs) &&
            obj.yatPermDutyClaimDTOs.length > 0
              ? obj.yatPermDutyClaimDTOs[0]
              : null;
          ...
          this.claims = {
            ...this.claims,
            ...obj,
            yatPermDutyClaimDTOs: [{ ...existingAdv, ...(dto || {}) }],
          ...
          this.calculateAmount(this.codeClaim.pmtClm);
          this.checkForPreviewBtn();
        },
        (error) => {
          this.$common.hideLoader();
      ...
      this.$common.hideLoader();
      console.error('Error while loading Resettlement Claim', e);
      this.$common.showMessage('Error while loading Resettlement Claim.', 'danger');
    }
  }
  ...
  getUnits(): void {
    try {
      ...
      this.$codeUnitApi.getAllUnits(config).subscribe(
        ...
        (err) => {
          this.$common.hideLoader();
          console.error('Error while loading Resettlement units.', err);
        }
      );
    } catch (error) {
      this.$common.hideLoader();
      console.error('Error while loading Resettlement units.', error);
    }
  }
  ...
  calculateAmount(type: string): void {
    if (type !== this.codeClaim.pmtClm) return;
    ...
    this.calculateNintyAmount(this.codeClaim.pmtClm);
  ...
  private getPmtAdv() {
    return this.claims?.yatPermDutyClaimDTOs?.[0];
  }
  ...
  saveClaim(type: string, status: string, showToast: boolean = true): void {
    if (type !== this.codeClaim.resettleClm) return;
    ...
    try {
      ...
      tempClaim.yatPermDutyClaimDTOs = Array.isArray(tempClaim.yatPermDutyClaimDTOs)
        ? tempClaim.yatPermDutyClaimDTOs
        : [tempClaim.yatPermDutyClaimDTOs];
      ...
      tempClaim.yatPermDutyClaimDTOs.forEach((adv: any) => {
  ...
  submitPmt(): void {
    ...
    const okSections = this.runResettlementClientValidationOnly();
    if (!okSections) return;

    const okBusiness = this.validateResettlementBusinessFields();
  ...
  validateResettlement(type?: string): void {
    if (type && type !== this.codeClaim.resettleClm) return;

    try {
      const okSections = this.runResettlementClientValidationOnly();
      if (!okSections) return;

      const okBusiness = this.validateResettlementBusinessFields(false);
      if (!okBusiness) return;
      ...
      console.error('Error during Resettlement validation', err);
    }
  }

  private runResettlementClientValidationOnly(): boolean {
    const sectionIds = this.getTabOrder();
  ...
  private validateResettlementBusinessFields(showSuccessMessage = false): boolean {
    const adv = this.claims?.yatPermDutyClaimDTOs?.[0];
    if (!adv) {
      this.$common.showMessage('Resettlement details are missing.', 'danger');
    ...
    this.$esignApi.checkEsignAvailability(config).subscribe({
      next: (response: any) => {
        if (response?.status === false) {
          ...
          this.$common.showMessage(
            response?.message || 'eSign is not available for this Resettlement claim.',
            'danger'
          );
        }
      },
  ...
  goToTab(tabId: 'ship' | 'ship2' | 'ship3' | 'ship4' | 'ship5' | 'ship6' | 'ship7'): void {
    this.setTab(tabId);
  }

  private getTabOrder(): Array<'ship' | 'ship2' | 'ship3' | 'ship4' | 'ship5' | 'ship6' | 'ship7'> {
    return ['ship', 'ship2', 'ship4', 'ship5', 'ship7', 'ship6'];
  }

  openTab(tabId: string): void {
    this.setTab(tabId as 'ship' | 'ship2' | 'ship3' | 'ship4' | 'ship5' | 'ship6' | 'ship7');
  }
  ...
  updateEntitlement(subFormId?: string): void {
    if (subFormId && subFormId !== this.codeClaim.resettleClm) return;
  ...
  private resolveSubFormId(rawSubFormId: any): string {
    ...
    return this.codeClaim.resettleClm;
  }
  ...
  private getCurrentFormRoute(): string {
    return 'form-resettlement-claim';
  }
  ...
  get adv(): any {
    return this.claims?.yatPermDutyClaimDTOs?.[0] ?? null;
  }
