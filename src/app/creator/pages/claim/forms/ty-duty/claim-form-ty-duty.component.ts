...
import { clearLegacyInvalidFromEvent, validateLegacyRequiredSection } from '../../../advance/forms/shared/helpers/legacy-form-validation.helper';
...
interface TyDutyClaimDTO {
  basicPay?: number | null;
  yatTempDutyClaimDTOs: TyDutyHeaderDTO[];
}
...
interface YatTempDutyClaimDTO {
  tempDutyClmId?: number | null;
...
interface Claims {
  ...
  yatTempDutyClaimDTOs: YatTempDutyClaimDTO[];
...
@Component({
  selector: 'app-claim-form-ty-duty',
...
export class ClaimFormTyDutyComponent implements OnInit {
  ...
  private readonly tyTabOrder: Array<'ship' | 'ship2' | 'ship3' | 'ship4' | 'ship5' | 'ship6' | 'ship7'> = [
    'ship',
    'ship2',
    'ship3',
    'ship4',
  ...
  codeClaim = {
    tyClm: 'TYD', // legacy TY Duty Claim sub-form id
  } as const;
  ...
  gxFormModel: any;
  showErrors: any;
  activeSubFormId: string = this.codeClaim.tyClm;
  ...
  private syncRouteStateFromQuery(qp: { get: (key: string) => string | null }): void {
    ...
    this.activeSubFormId = querySubFormId || routeSubFormId || this.codeClaim.tyClm;
  ...
  private createEmptyClaims(): Claims {
    return {
      ...
      yatTempDutyClaimDTOs: [this.createEmptyTyClaim()],
      yatDtsDetailDTOs: [],
  ...
  private createEmptyTyClaim(): YatTempDutyClaimDTO {
    return {
      tempDutyClmId: null,
      pno: null,
      name: null,
  ...
  calculateAmount(type: string, isTotalAccHToDutyChanged = false): void {
    if (type !== this.codeClaim.tyClm) {
      return;
    }

    const adv = this.claims.yatTempDutyClaimDTOs[0];
    if (!adv) {
      return;
    }
  ...
  private normalizeFinancialInputs(adv: YatTempDutyClaimDTO): void {
  ...
  onFoodRequiredChange(val: any): void {
    ...
    this.calculateAmount(this.codeClaim?.tyClm);
  }
  ...
  private mapTyPersonalDetails(adv: YatTempDutyClaimDTO, source: any, _root: any): void {
  ...
  private mergeSavedTyResponse(savedClaim: any): void {
    ...
    const currentAdv = this.claims.yatTempDutyClaimDTOs?.[0] || {};
    const savedAdv = this.firstObject(savedClaim.yatTempDutyClaimDTOs);
    this.claims = {
      ...this.claims,
      ...savedClaim,
      yatTempDutyClaimDTOs: [
        {
          ...currentAdv,
          ...savedAdv,
        },
  ...
  setTab(tab: 'ship' | 'ship2' | 'ship3' | 'ship4' | 'ship5' | 'ship6' | 'ship7'): void {
    this.activeTab = tab;
  }

  private activateTab(sectionId: string): void {
    if (this.tyTabOrder.includes(sectionId as any)) {
      this.setTab(sectionId as 'ship' | 'ship2' | 'ship3' | 'ship4' | 'ship5' | 'ship6' | 'ship7');
    }
  }
      ...
      this.$claimApi.getSingleClaim(config).subscribe(
        (response: any) => {
          ...
          if (!response || response.status !== true) {
            ...
            if (this.claimIdParam) {
              this.$common.showMessage(
                response?.message || 'Unable to load TY Duty Claim details.',
                'danger'
              );
            }
            return;
          ...
          const dto =
            Array.isArray(obj.yatTempDutyClaimDTOs) &&
            obj.yatTempDutyClaimDTOs.length > 0
              ? obj.yatTempDutyClaimDTOs[0]
              : null;
          ...
          if (dto) {
            ...
            if (Array.isArray(dto.unitList)) {
              ...
              Array.isArray(obj.yatTempDutyClaimDTOs) &&
              obj.yatTempDutyClaimDTOs.length > 0 &&
              Array.isArray(obj.yatTempDutyClaimDTOs[0].unitList)
            ) {
              adv.unitList = obj.yatTempDutyClaimDTOs[0].unitList;
            } else {
              adv.unitList = [];
            }
            ...
            // Legacy TY advance fills personal fields from yatTempDutyClaimDTOs[0].
            // Avoid flattened/user-root fallbacks here so Advance and Claim
            // contracts stay separate.
            this.mapTyBankDetails(obj);
          }

          const mappedAdv = this.claims.yatTempDutyClaimDTOs?.[0];
          this.changeTransType(mappedAdv?.tempTransferToType || null, this.codeClaim.tyClm);
        ...
        (error) => {
          this.$common.hideLoader();
          console.error('Error while loading TY Duty Claim details.', error);
          this.$common.showMessage(
            'Error while loading TY Duty Claim details.',
            'danger'
          );
        }
      );
  ...
  checkValidSignType(signWith: string | null, appliedTo: string | null, getCalled: boolean = false): void {
    ...
    const unitName =
      appliedTo ||
      this.claims?.yatTempDutyClaimDTOs?.[0]?.appliedTo ||
  ...
  navigatePreview(type, id) {
    ...
    const isClaimPreview = route.includes('claim');
  ...
  private completeLegacyStatusTransition(status: string, claimId: string): void {
    if (!claimId) {
      this.$common.showMessage(
        'Unable to update TY Duty claim status.',
        'danger'
      );
      this.$common.hideLoader();
    ...
    this.$esignApi.checkEsignAvailability(config).subscribe({
      next: (response: any) => {
        if (response?.status === false) {
          ...
          this.$common.showMessage(
            response?.message ||
              'eSign is not available for this TY Duty claim.',
            'danger'
          );
        }
      },
  ...
  private buildTySavePayload(status: string): any {
    ...
    tempClaim.claimAmt = tempClaim.yatTempDutyClaimDTOs?.[0]?.advAmt ?? 0;

    if (tempClaim.yatTempDutyClaimDTOs) {
      if (!Array.isArray(tempClaim.yatTempDutyClaimDTOs)) {
        tempClaim.yatTempDutyClaimDTOs = [tempClaim.yatTempDutyClaimDTOs];
      }
    } else {
      tempClaim.yatTempDutyClaimDTOs = [];
    }
    tempClaim.yatTempDutyClaimDTOs.forEach((adv: any) => {
      ...
      this.$claimApi.createOrUpdateClaim(formData, null).subscribe(
        (res: any) => {
          ...
          if (respStatus === this.codeClaimState.outbox) {
            this.$common.showMessage(
              res?.message ||
                'TY Duty claim submitted successfully!',
              'success'
            );
          } else if (respStatus === this.codeClaimState.draft) {
            this.$common.showMessage(
              res?.message ||
                'TY Duty claim draft saved successfully.',
              'success'
            );
          }
  ...
  private getCurrentFormRoute(): string {
    return 'form-ty-duty-claim';
  }

  private getPreviewRoute(): string {
    return 'preview-ty-duty-claim';
  }
  ...
  private runTyClientValidationOnly(): boolean {
    const sectionIds = ['ship', 'ship2', 'ship3', 'ship4', 'ship5', 'ship7', 'ship6'];
  ...
  private runTyServerValidation(status: string, onSuccess?: () => void): void {
    ...
    this.$validationApi.validateClaims(validationPayload).subscribe(
      ...
      (error) => {
        this.$common.hideLoader();
        console.error('Error while validating TY Duty Claim.', error);
        this.$common.showMessage('Error while validating TY Duty Claim.', 'danger');
      }
    );
  }

  validateTy(type: string): void {
    if (type !== this.codeClaim.tyClm && type !== this.activeSubFormId) return;
  ...
  submitTy(): void {
    ...
    this.runTyServerValidation(this.codeClaimState.outbox, () => {
      this.saveClaim(this.codeClaim.tyClm, this.codeClaimState.outbox, true);
    });
  }
