...
export class ClaimFormPmtDutyComponent implements OnInit {
  ...
  activeSubFormId: string = this.codeClaim.pmtClm;
  ...
  getFormDetails(): void {
    try {
      ...
      this.$common.hideLoader();
      console.error('Error while loading PMT Duty Claim', e);
      this.$common.showMessage('Error while loading PMT Duty Claim.', 'danger');
    }
  }
  ...
  validatePmt(type?: string): void {
    if (type && type !== this.codeClaim.pmtClm) return;
  ...
  updateEntitlement(subFormId?: string): void {
    if (subFormId && subFormId !== this.codeClaim.pmtClm) return;
  ...
  private resolveSubFormId(rawSubFormId: any): string {
    ...
    if (id === 'PMT' || id === 'PMTCLM' || id === 'PMTA' || id === 'P') return this.codeClaim.pmtClm;
    return this.codeClaim.pmtClm;
  }
  ...
  private getCurrentFormRoute(): string {
    return 'form-pmt-duty-claim';
  }

  private getPreviewRoute(): string {
    return 'preview-pmt-duty-claim';
  }

  private getFormDisplayName(): string {
    return 'PMT Duty claim';
  }
