...
@Component({
    ...
    styleUrls: ['./pilotage-history-status.component.scss'],
    standalone: false
})
...
codeStatus: { activate: string; deactivate: string; pending: string; approved: string; rejected: string; success: string; processing: string; cancel: string; outbox: string; draft: string; inbox: string; };
  ...
  permissionCreate() {
    try {
      ...
      this.$pilotageService.permissionCreate(this.config).subscribe(
        (response: any) => {
          this.$common.hideLoader();
          if (response.status === true) {
            let isAllow = response.object;
            this.isAllowedCreateHis = isAllow;
          }
        },
        (err) => {
          this.$common.hideLoader();
